...
export class DrawCardV2 extends Component {
    ...
    start() {
        ...
        let self = this;
        // 注册监听
        let touchBegan = function (evt: EventTouch) {
        ...
        let touchMove = function (evt: EventTouch) {
            let pos = evt.getUILocation();
            ...
            if (self.touchFirstPos) {
                ...
                self.runRotationAction(self.rotateFirstPos, pos);
            } else {
                let touchTp = self.getTouchTypeByNode(pos, self._backMaterialNode);
                //如果是边缘范围,把当前点击的坐标赋值给this.touchFirstPos
        ...
        let touchEnded = function (evt: EventTouch) {
            self.touchFirstPos = null;
            self.rotateFirstPos = null;
            self.resetPos();
        }
        let touchCancel = function (evt: EventTouch) {
            self.touchFirstPos = null;
            self.rotateFirstPos = null;
            self.resetPos();
        }
    ...
    getAngleByPos(pos1: Vec2, pos2: Vec2) {
        ...
        return 180 / Math.PI * tanValue;
    }

    // 设置旋转角度
    runRotationAction(firstPos: Vec2, secondPos: Vec2) {
